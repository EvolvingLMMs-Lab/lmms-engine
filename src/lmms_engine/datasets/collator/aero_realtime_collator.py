...
class AeroRealtimeCollator(VisionCollator):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        for key in ("codec_labels", "codec_input_ids"):
            codec_list = inputs.pop(key, None)
            if codec_list is None:
                continue
            max_len = max(c.shape[0] for c in codec_list)
            G = codec_list[0].shape[1]
            ...
            for c in codec_list:
                ...
                padded.append(c)
            batched_inputs[key] = torch.stack(padded, dim=0)
