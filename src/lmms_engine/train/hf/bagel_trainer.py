        ...
        if ce is not None:
            ...
            if ce_loss_indexes is not None:
                total_ce_tokens = torch.tensor(
                    ce_loss_indexes.sum().item(), device=device
                )
            else:
                # fallback: 使用实际ce loss的数量
                total_ce_tokens = torch.tensor(
                    ce.numel() if ce.numel() > 0 else 1, device=device
                )
            ...
            if ce_loss_weights is not None and getattr(
                self.args, "ce_loss_reweighting", False
            ):
        ...
        if mse is not None:
            ...
            if mse_loss_indexes is not None:
                total_mse_tokens = torch.tensor(
                    mse_loss_indexes.sum().item(), device=device
                )
            else:
                # fallback: 使用实际mse loss的数量
                total_mse_tokens = torch.tensor(
                    mse.numel() if mse.numel() > 0 else 1, device=device
                )
