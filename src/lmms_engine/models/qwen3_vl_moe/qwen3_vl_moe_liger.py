...
def lce_forward(
    ...
    **kwargs,
    ...
    output_router_logits = (
        output_router_logits if output_router_logits is not None else getattr(self.config.text_config, "output_router_logits", False)
    )

    tokens_count = attention_mask.sum().item() if attention_mask is not None else 0
    n_image_tokens = (
