# Adapted from https://github.com/JiuhaiChen/BLIP3o/blob/BLIP3o-NEXT/blip3o/model/multimodal_encoder/ta_tok_encoder.py
...
class TATokVisionTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        self.vision_tower = TextAlignedTokenizer.from_checkpoint(
            self.vision_tower_name, load_teacher=False
        ).to(device_map)
        self.vision_tower.bottleneck.regularizer.set_eval_deterministic(
            deterministic=True
        )

        self.vision_tower.input_type = "rec"
        self.vision_tower.scale_layer = ScalingLayer(
    ...
    def forward(self, images, pool_scale=1):
        ...
        if type(images) is list:
            ...
            for image in images:
                ...
                image_feature, token = (
                    image_forward_out["vq_feats"].to(image.dtype),
                    image_forward_out["bottleneck_rep"],
                )
                image_features.append(image_feature)
                tokens.append(token)
        else:
            image_forward_outs = self.vision_tower(
                images.to(device=self.device, dtype=self.dtype), pool_scale=pool_scale
            )
            image_features, tokens = (
                image_forward_outs["vq_feats"].to(images.dtype),
                image_forward_outs["bottleneck_rep"],
            )
        return {
