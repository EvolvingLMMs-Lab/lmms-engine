...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos, scale_factor=10000):
    ...
    omega /= embed_dim / 2.0
    omega = 1.0 / scale_factor**omega  # Parameterized scaling factor (D/2,)
...
class KVCache(nn.Module):
    ...
    def update(self, input_pos, k_val, v_val):
        ...
        ), f"{input_pos.shape[0]} != {k_val.shape[2]}"
        k_out = self.k_cache
        v_out = self.v_cache
...
class ARModel(nn.Module):
    def __init__(self, config: ModelArgs):
        ...
        if config.use_fixed_pe:
            self.register_buffer(
                "abs_pe",
                torch.zeros(
                    1, config.max_seq_len + config.cls_token_num - 1, config.dim
                ),
            )
            abs_pe = get_1d_sincos_pos_embed_from_grid(
                embed_dim=config.dim,
                pos=np.arange(config.max_seq_len + config.cls_token_num - 1),
            )
    ...
    def initialize_weights(self):
        ...
        if hasattr(self.output, "weight") and isinstance(
    ...
    def setup_caches(self, max_batch_size, max_seq_length, dtype):
        assert (
            max_seq_length == self.max_seq_length + self.cls_token_num
    ...
    def forward(
        ...
        if idx is not None and cond_idx is not None:  # training or naive inference
            ...
            token_embeddings = torch.cat(
                (cond_embeddings, token_embeddings), dim=1
            )  # (b, cls_token_num + n, d)
