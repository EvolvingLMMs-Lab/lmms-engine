...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16", attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if overwrite_config is not None:
        ...
        model = blip3oQwenForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, dtype=torch_dtype, attn_implementation=attn_implementation, config=blip3o_cfg, **kwargs)
    else:
        model = blip3oQwenForCausalLM.from_pretrained(model_path, attn_implementation=attn_implementation, dtype=torch_dtype, **kwargs)
