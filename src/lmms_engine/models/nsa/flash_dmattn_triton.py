...
## Copied from https://github.com/SmallDoges/flash-dmattn/blob/main/flash_dmattn/flash_dmattn_triton.py
# Adapted with attention bias removed
...
def _fwd_kernel(
    ...
    # Initialize pointers to Q, K, V, Mask
    ...
    for start_n in range(0, end_n, BLOCK_N):
        ...
        # can then fuse the mult and add into an fma instruction.
...
def _bwd_kernel_one_col_block(
    ...
    for start_m in range(begin_m, num_block_m * BLOCK_M, BLOCK_M):
        ...
        if not EVEN_HEADDIM:
            tl.debug_barrier()
        # compute ds = p * (dp - delta[:, None]) * softmax_scale
        ...
        # compute dq
        if not (EVEN_M & EVEN_HEADDIM):  # Otherewise there's a race condition
            tl.debug_barrier()
        if not ATOMIC_ADD:
...
def _flash_attn_forward(q, k, v, mask, softmax_scale=None, is_causal=False):
    # shape constraints
    ...
    assert mask.shape == (
        ...
        seqlen_q,
        seqlen_k,
    ), f"mask shape {mask.shape} does not match expected shape {(batch, nheads, seqlen_q, seqlen_k)}"
    assert mask.dtype in [
        torch.float16,
        torch.bfloat16,
        torch.float32,
    ], "mask must be fp16, bf16, or fp32"
...
def _flash_attn_backward(
    do, q, k, v, mask, o, lse, softmax_scale=None, is_causal=False
):
...
class FlashDMAttnFunc(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, query, key, value, attn_mask=None, is_causal=False, softmax_scale=None
    ):
        """
        ...
        query, key, value, attn_mask = [
            ...
            for x in [query, key, value, attn_mask]
        ]
        ...
        ctx.save_for_backward(query, key, value, o, lse, attn_mask)
        ctx.is_causal = is_causal
        return o

    @staticmethod
    def backward(ctx, do):
        query, key, value, o, lse, attn_mask = ctx.saved_tensors
        assert not ctx.needs_input_grad[
            3
        ], "FlashDMAttn does not support mask gradient yet"
...
def triton_dmattn_func(query, key, value, attn_mask=None, is_causal=False, scale=None):
    return FlashDMAttnFunc.apply(query, key, value, attn_mask, is_causal, scale)
