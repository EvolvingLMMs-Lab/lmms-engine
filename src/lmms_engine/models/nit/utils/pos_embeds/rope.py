...
def get_proportion(L_test, L_train):
    L_test = L_test * 2
    return torch.where(
        torch.tensor(L_test / L_train) <= 1.0,
        torch.tensor(1.0),
        torch.sqrt(torch.log(torch.tensor(L_test)) / torch.log(torch.tensor(L_train))),
    )
...
class VisionRotaryEmbedding(nn.Module):
    def __init__(
        ...
        if not online_rope:
            if self.custom_freqs in ["normal", "scale1", "scale2"]:
                freqs_h = 1.0 / (theta ** (torch.arange(0, dim, 2).float() / dim))
                freqs_w = 1.0 / (theta ** (torch.arange(0, dim, 2).float() / dim))
            else:
                if decouple:
            ...
            if max_pe_len_h != None and max_pe_len_w != None and ori_max_pe_len != None:
                ...
                scale = torch.clamp_min(
                    torch.tensor(max(max_pe_len_h, max_pe_len_w)) / ori_max_pe_len, 1.0
                )  # dynamic scale
                self.mscale = (
                    get_mscale(scale).to(scale) * attn_factor
    ...
    def get_1d_rope_freqs(self, theta, dim, max_pe_len, ori_max_pe_len):
        ...
        if self.custom_freqs == "linear":  # equal to position interpolation
            freqs = 1.0 / torch.einsum("..., f -> ... f", scale, theta ** (torch.arange(0, dim, 2).float() / dim))
        elif (
            self.custom_freqs == "ntk-aware"
            or self.custom_freqs == "ntk-aware-pro1"
            or self.custom_freqs == "ntk-aware-pro2"
        ):
            freqs = (
                1.0
                / torch.pow(
                    find_newbase_ntk(dim, theta, scale).view(-1, 1), (torch.arange(0, dim, 2).to(scale).float() / dim)
                ).squeeze()
            )
            ...
            freqs_linear = 1.0 / torch.einsum(
                "..., f -> ... f", scale, (theta ** (torch.arange(0, dim, 2).to(scale).float() / dim))
            )
            freqs_ntk = (
                1.0
                / torch.pow(
            ...
            freqs_interpolation = 1.0 / torch.einsum(
                "..., f -> ... f", scale, (theta ** (torch.arange(0, dim, 2).to(scale).float() / dim))
            )

            low, high = find_correction_range(beta_fast, beta_slow, dim, theta, ori_max_pe_len)
            freqs_mask = (
                1 - linear_ramp_mask(low, high, dim // 2).to(scale).float()
            ...
            raise ValueError(
                f"Unknown modality {self.custom_freqs}. Only support normal, linear, ntk-aware, ntk-by-parts, yarn!"
            )
        return freqs
