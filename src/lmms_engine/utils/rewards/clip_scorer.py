...
class ClipScorer(torch.nn.Module):
    def __init__(self, device):
        super().__init__()
        self.device = device
        self.model = CLIPModel.from_pretrained("/pfs/training-data/kemingwu/hf_cache/models/clip-vit-large-patch14").to(
            device
        )
        self.processor = CLIPProcessor.from_pretrained(
            "/pfs/training-data/kemingwu/hf_cache/models/clip-vit-large-patch14"
        )
